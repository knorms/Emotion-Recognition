...
function [finalData, failed, averageFace]  = clmDataInit(num_extreme_frames, fileStructArray)
    ...
    firstFace = fileStructArray(1,1).data;
    averageFace = firstFace;
    
    for n = 5:4:inputSize
        neutral = fileStructArray(1,n).data;
        [~, n_align] = procrustes(firstFace, neutral); 
        averageFace = averageFace + n_align;
    end
    averageFace = averageFace/(inputSize/4);
    ...
    for n = 1:4:inputSize
        ...
        if(extreme1.data(1,1) ~= 0) % failure to converge
            ...
            [~, e_align1] = procrustes(averageFace,extreme1.data);
            myStruct1 = struct('neutral', neutral.data, 'extreme', extreme1.data, 'indicator', 1, ...
                               ...
                               'e_align', e_align1, 'difference', averageFace - e_align1);
        ...
        if(extreme2.data(1,1) ~= 0)
            if (num_extreme_frames>=2) % keeping at least last 2 frames
                [~, e_align2] = procrustes(averageFace,extreme2.data);
                myStruct2 = struct('neutral', neutral.data, 'extreme', extreme2.data, 'indicator', 2, ...
                                   ...
                                   'e_align', e_align2, 'difference', averageFace - e_align2);
        ...
        if(extreme3.data(1,1) ~= 0) % failure to converge, always keep peak frame
            [~, e_align3] = procrustes(averageFace,extreme3.data);
            myStruct3 = struct('neutral', neutral.data, 'extreme', extreme3.data, 'indicator', 3, ...
                               ...
                               'e_align', e_align3, 'difference', averageFace - e_align3);
    ...
    finalData = clmDataPreprocessing(data);
end
