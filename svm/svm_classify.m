function [predicted_emotions, data_val, category_lbls, perc_corr, C, Mdl, percentCorrectByEmotion] = svm_classify(data)
...
data_train = data(arr(1:floor(length(data)*0.80)));
data_val = data(arr((floor(length(data)*0.80))+1:end));
...
for i = 1:numel(categories)
    ...
    Mdl{i} = fitcsvm(train_feats, lbls, 'Standardize', true, ...
...
correctByEmotion = [0, 0, 0, 0, 0, 0, 0];
totalByEmotion = [0, 0, 0, 0, 0, 0, 0];
perc_corr = sum(val_labels == predicted_emotions)/length(predicted_emotions);
for i=1:length(predicted_emotions)
    if val_labels(i) == predicted_emotions(i)
        correctByEmotion(val_labels(i)) = correctByEmotion(val_labels(i)) + 1;        
    end
    totalByEmotion(val_labels(i)) = totalByEmotion(val_labels(i)) + 1;
end
percentCorrectByEmotion = correctByEmotion ./ totalByEmotion;
%  
